...
# Set host and port before Streamlit initializes
os.environ["STREAMLIT_SERVER_PORT"] = "8533"         # Set your custom port
os.environ["STREAMLIT_SERVER_ADDRESS"] = "0.0.0.0"    # Optional: for network access
...
# Page Configuration
st.set_page_config(page_title="DocZ", page_icon="🚀", layout="centered", menu_items={})
st.title("💬 AI Chatbot")
...
def sidebar_navigation():
    if not st.session_state.logged_in:
        # if st_javascript("window.innerWidth") > 768:
        ...
        if st.session_state.username:
            ...
            if st.sidebar.button("🆕 New Chat"):
                ...
                st.query_params.update({
                                        "CHAT-ID": st.session_state.active_chat
                                    })
...
def chat_interface():
    if st.session_state.logged_in:
        ...
        if st.session_state.active_chat is None:
            st.session_state.active_chat = st.query_params.get("CHAT-ID","")
            # st.query_params.update({
            #                             "CHAT-ID": st.session_state.active_chat
            #                         })
            ...
            if active_chat_id in st.session_state.chats:
                messages = st.session_state.chats[active_chat_id]
                for message in messages:
                    with st.chat_message(message["role"]):
                        st.markdown(message["content"])
            else:
                # st.warning("Invalid chat ID selected,Please create new chat.")
            ...
            if active_chat_id in st.session_state.chats:
                ...
                st.warning("Invalid chat ID selected2.")
    else:
        # st.set_page_config(page_title="DocZ", page_icon="🚀", layout="centered", menu_items={})
        st.sidebar.warning("Please log in to start chatting.")
